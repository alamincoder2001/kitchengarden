						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									...
									<div class="col-xs-9">
									<v-select v-bind:options="categories" v-model="selectedCategory" label="ProductCategory_Name" v-on:input="categoryOnChange"></v-select>
									</div>
								</div>
	...
	new Vue({
		...
		methods: {
			...
			categoryOnChange(){
				...
				this.products = this.products1.filter(p => p.ProductCategory_ID == this.selectedCategory.ProductCategory_SlNo)
			},
			getEmployees() {
			...
			getProducts() {
				axios.post('/get_products', {
					...
					if (this.sales.salesType == 'wholesale') {
						this.products1 = res.data.filter((product) => product.Product_WholesaleRate > 0);
